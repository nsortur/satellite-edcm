...
class AttrMessagePassing(torch.nn.Module):
  def __init__(
    ...
    for li, (irreps_node_hidden, irreps_edge_attr) in enumerate(zip(irreps_node_sequence[1:-1], self.irreps_edge_attrs[:-1])):
      ...
      #if li == 1:
      #  pool = e3nn_utils.VoxelPooling(gate.irreps_out, self.lmax, [0.5, 0.5, 0.5])#, start=[-2, -2, -2],end=[2, 2, 2])
      #else:
      #  pool = e3nn_utils.VoxelPooling(gate.irreps_out, self.lmax, [0.1, 0.1, 0.5])#, start=[-2, -2, -2],end=[2, 2, 2])
      #pool = e3nn_utils.EdgePooling(gate.irreps_out, self.lmax)
      #pool = e3nn_utils.TopKPooling(gate.irreps_out, self.lmax)
      pool = None
