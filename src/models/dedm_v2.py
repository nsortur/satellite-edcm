...
class E3GNNConv(torch.nn.Module):
    ...
    def forward(self, node_in, edge_src, edge_dst, edge_attr, edge_len_emb):
        ...
        agg_messages = torch_scatter_sum(
            messages, edge_dst, dim=0, dim_size=node_in.shape[0]
        )
...
class DEDMV2(torch.nn.Module):
    ...
    def forward(self, data):
        pos, node_features, batch = (
            data["pos"],
            data["x"],
            data.get("batch", torch.zeros_like(data["pos"][:, 0], dtype=torch.long)),
        )
        ...
        edge_attr = o3.spherical_harmonics(
            self.irreps_edge_attr, edge_vec, True, "component"
        )
        edge_len_emb = soft_one_hot_linspace(
            edge_len, 0.0, self.max_radius, 10, "smooth_finite", True
        ) * (10**0.5)
        ...
        sh_query = o3.spherical_harmonics(
            self.irreps_s2_out, query_vector, False, "component"
        )
...
if __name__ == "__main__":
    ...
    ds = DragMeshDataset(
        "/Users/neelsortur/Documents/codestuff/sat-modeling/satellite-edcm/data/cube50k.dat",
    ...
    model = DEDMV2(o3.Irreps("5x0e"), lmax=LMAX)
    output = model(data)
