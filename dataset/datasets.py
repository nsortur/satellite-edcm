...
class DragDataset(torch.utils.data.Dataset):
    def __init__(self, file_name, norm_features, DEVICE="cpu"):
        ...
        df = pd.read_csv(
            utils.to_absolute_path(file_name), delim_whitespace=True, header=None
        )
        df = df[:-1]
        ...
        if norm_features:
            in_vars = (in_vars - in_vars.min(axis=0)) / (
                in_vars.max(axis=0) - in_vars.min(axis=0)
            )
...
class DragMeshDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.vertices_canonical = torch.tensor(
            mesh.vertices, dtype=torch.get_default_dtype()
        ).to(DEVICE)
        ...
        df = pd.read_csv(
            utils.to_absolute_path(attr_file), delim_whitespace=True, header=None
        )
        df = df[:-1]
        ...
        if norm_features:
            attrs = (attrs - attrs.min(axis=0)) / (
                attrs.max(axis=0) - attrs.min(axis=0)
            )
        self.xs = torch.tensor(attrs, dtype=torch.float32).to(DEVICE)

        # needs to be CPU for D_from_angles
        self.orientation = torch.tensor(df.iloc[:, 5:7].values, dtype=torch.float32).to(
            DEVICE
        )
    ...
    def __getitem__(self, idx):
        # calculate edge_vec
        edge_vec = (
            self.vertices_canonical[self.edges[:, 1]]
            - self.vertices_canonical[self.edges[:, 0]]
        )
        if self.return_features_separately:
            data = Data(
                pos=self.vertices_canonical,
                x=torch.ones_like(self.xs[idx]).repeat(
                    self.vertices_canonical.size(0), 1
                ),
            ...
            data = Data(
                ...
                x=torch.cat(
                    (
                        self.xs[idx].repeat(self.vertices_canonical.size(0), 1),
                        self.orientation[idx].repeat(
                            self.vertices_canonical.size(0), 1
                        ),
                    ),
                    dim=1,
...
if __name__ == "__main__":
    ...
    ds = DragMeshDataset(
        "data/cube50k.dat", "STLs/Cube_38_1m.stl", orientation_features=True
    )
    print(ds[2])
